...
With Euiwoong Lee, I studied the Unique Games problem when the constraint graph is a metric graph, i.e.\ a complete graph equipped with a weight that obeys the triangle inequality. The Unique Games problem is central in the theory of hardness of approximation; it is conjectured to admit no $O(1)$-approximation in the general case. This conjecture implies the optimality of approximation algorithms for several canonical $\NP$-hard graph problems, including \textsc{VertexCover} and \textsc{MaxCut}. Understanding the approximability of the problem on more restricted instances such as ours can give insight into the general case. 
...
I look forward to working with Professor Julia Chuzhoy on approximation algorithms for combinatorial optimization problems, particularly for graph problems. Her recent work in graph algorithms intersects nicely with my burgeoning interest in parameterized algorithms. I would also be excited to work with her on problems related to hardness of approximation, whether that be through improving existing lower bounds through new hardness assumptions, or by giving improved approximation algorithms. I would also look forward to working with Professor Yury Makarychev on approximation algorithms, particularly on problems related to CSPs and on algorithms that use techniques from continuous optimization for combinatorial problems.
